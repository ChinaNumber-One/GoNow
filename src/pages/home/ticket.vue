<template>
	<div class="ticket">
		...
		<div class="city">
			<router-link :to="{ name: 'city', params: { type: 'from' }}" class="startPlace" tag="div">{{startPlace}}</router-link>
			<img src="../../../static/img/fromto.png" alt="">
			<router-link :to="{ name: 'city', params: { type: 'to' }}" class="startPlace" tag="div">{{endPlace}}</router-link>
		</div>
		<div class="date-pick">
			<date-picker 
          v-model="time1" 
      ...
      <date-picker v-model="time2" :not-before="today" :not-after="endDay"  v-bind:style="{ width: '40%', marginRight: '2%', float: 'right', marginTop: '.16rem' }" v-show="!isActive"></date-picker>
		...
		<div class="search">
			<div class="search-btn" @click="searchTicket">搜索</div>
		</div>
